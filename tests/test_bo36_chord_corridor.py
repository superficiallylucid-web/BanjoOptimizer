...
# Phase 1 migration (BO-149): FULL_SONG_PATH now resolves via
# the canonical fixture_path() (test_scores/), not a bare
# filename relative to whatever the current working directory
# happened to be -- confirmed directly this fixture previously
# had to live in the project ROOT itself (not scores/) for this
# specific bare-filename convention to resolve at all.
...
def test_full_pipeline_real_example_and_consistency():
    ...
    # Phase 1 migration (BO-149): writes to test_output/ (via
    # new_output_dir()) instead of the real application output/
    # directory -- confirmed generate_tab_from_template() itself
    # treats output_folder as a plain, arbitrary path, with no
    # hardcoded "output" behavior of its own.
