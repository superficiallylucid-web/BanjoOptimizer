...
def _nearby_melody_pitches(harmony, melody_notes):
    """
    BO-39: the exact MIDI pitch(es) of the nearest melody note
    immediately BEFORE this harmony's own onset and the nearest
    melody note immediately AFTER it -- regardless of whether
    either one is itself a chord onset. Used only to WIDEN
    candidate generation (chord_service.get_shapes_for_exact_
    melody_pitch()'s own widening_pitches parameter) when this
    harmony has no melody note at its own exact onset at all
    (the real, confirmed C7 example) -- these are deliberately
    kept separate from BO-20's own onset-exact melody_pitches,
    since they are not actually sounding at this chord's own
    location, only near it.

    Returns a (possibly empty) set of MIDI values -- up to two
    (one before, one after); either or both may be absent (e.g.
    a harmony at the very start or end of the piece).
    """
    ...
    ordered = sorted(
        melody_notes, key=lambda note: (note.measure, note.beat)
    )

    harmony_key = (harmony.measure, harmony.beat)
    ...
    for note in ordered:
        ...
        note_key = (note.measure, note.beat)

        if note_key < harmony_key:

            nearest_before = note

        elif note_key > harmony_key and nearest_after is None:

            nearest_after = note

    if nearest_before is not None:

        pitches.add(nearest_before.midi)

    if nearest_after is not None:

        pitches.add(nearest_after.midi)
...
def _select_chord_shape_for_harmony(
    ...
    preceding_chord_working_fret=None,
    following_chord_working_fret=None
):
    """
    ...
    preceding_chord_working_fret / following_chord_working_
    fret: BO-39, optional -- forwarded unchanged to chord_
    service.get_shapes_for_exact_melody_pitch()'s own new
    tiebreak (see that function's own docstring for the min-
    distance combination when both are given). See generate_
    tab_from_template()'s own two-pass orchestration for how a
    real caller computes these without a circular dependency
    (an adjacent chord's own shape must already be known before
    it can inform this one). Both None (the default) reproduces
    this function's own prior behavior exactly -- every existing
    caller that doesn't pass either is completely unaffected.
    """
    ...
    if melody_pitches:
        ...
        shapes = chord_service.get_shapes_for_exact_melody_pitch(
            ...
            preceding_chord_working_fret=(
                preceding_chord_working_fret
            ),
            following_chord_working_fret=(
                following_chord_working_fret
            )
        )
        ...
        # BO-39: no melody note at this chord's own exact onset
        # -- the real, confirmed C7 example. The nearest
        # surrounding melody notes (immediately before/after,
        # regardless of whether either is itself a chord onset)
        # are passed as widening_pitches -- generation-widening
        # ONLY, never onset-containment ranking (they are not
        # actually at this chord's own beat) -- see chord_
        # service.get_shapes_for_exact_melody_pitch()'s own
        # docstring for exactly why these are kept distinct from
        # melody_pitches.
        widening_pitches = _nearby_melody_pitches(
            harmony, melody_notes
        ) if melody_notes is not None else None
        ...
        shapes = chord_service.get_shapes_for_exact_melody_pitch(
            ...
            widening_pitches=widening_pitches
        )
...
def _apply_chord_shapes(
    ...
    melody_notes=None, precomputed_selection_by_position=None
):
    """
    ...
    precomputed_selection_by_position: BO-39, optional -- a dict
    of (measure, beat) -> (chosen_shape, is_exception,
    exception_dict), exactly the return shape of _select_chord_
    shape_for_harmony(). When given, this function REUSES that
    exact selection for a matching harmony instead of calling
    _select_chord_shape_for_harmony() itself -- confirmed
    necessary, not merely an optimization: generate_tab_from_
    template() computes its own adjacent-chord-aware second pass
    (see that function's own docstring) purely for melody-
    position anchoring purposes; without reusing that same
    selection here, this function would independently re-select
    every chord's shape from scratch with NO adjacent-chord
    awareness at all, silently discarding the second pass's own
    work for the one place -- the actual FretDiagram -- where it
    matters most. A position missing from this dict (or the dict
    itself being None, the default) falls back to this function's
    own prior behavior for that harmony exactly, unaffected.
    ...
    for xml_harmony, harmony in zip(xml_harmony_elements, harmonies):
        ...
        precomputed = (
            precomputed_selection_by_position.get(
                (harmony.measure, harmony.beat)
            )
            if precomputed_selection_by_position is not None
            else None
        )
...
def generate_tab_from_template(
    ...
    # BO-39: a SECOND pass, re-selecting each harmony's own
    # shape using its immediate neighbors' own FIRST-pass working
    # frets as adjacent-chord context. Deliberately a separate
    # pass, not folded into the loop above -- avoids the circular
    # dependency a single pass would create (an adjacent chord's
    # own shape must already be known before it can inform this
    # one; the first pass above is exactly that independent
    # baseline). staff_harmonies is already in chronological
    # (document) order, so "the harmony immediately before/after
    # this one in that list" is simply the previous/next list
    # index -- no separate position-tracking system.
    first_pass_working_fret_by_position = {
        position: _chord_working_fret(values)
        for position, values in chord_shape_by_position.items()
    }

    second_pass_chord_shape_by_position = {}

    # BO-39: the FULL selection (shape, is_exception, exception_
    # dict) per position, not just the parsed fret values above
    # -- so _apply_chord_shapes() below can reuse this exact,
    # already-computed second-pass selection when writing the
    # actual FretDiagram XML, rather than independently re-
    # calling _select_chord_shape_for_harmony() without any
    # adjacent-chord awareness at all (which is what happened
    # before this dict existed -- confirmed as a real bug via
    # the real C7 example: chord_shape_by_position was already
    # correctly adjacent-chord-aware, but only ever consulted for
    # melody-position anchoring, never for the FretDiagram itself).
    second_pass_selection_by_position = {}

    for harmony_index, harmony in enumerate(staff_harmonies):

        preceding_working_fret = None

        if harmony_index > 0:

            preceding_harmony = staff_harmonies[
                harmony_index - 1
            ]

            preceding_working_fret = (
                first_pass_working_fret_by_position.get(
                    (
                        preceding_harmony.measure,
                        preceding_harmony.beat
                    )
                )
            )

        following_working_fret = None

        if harmony_index + 1 < len(staff_harmonies):

            following_harmony = staff_harmonies[
                harmony_index + 1
            ]

            following_working_fret = (
                first_pass_working_fret_by_position.get(
                    (
                        following_harmony.measure,
                        following_harmony.beat
                    )
                )
            )
    ...
    chord_shape_by_position = second_pass_chord_shape_by_position
    ...
    # BO-40: for every note event, the nearest chord onset AFTER
    # it (regardless of how many intervening notes sit between
    # them, the same wider-reaching lookup style BO-37 uses for
    # the preceding direction) together with that chord's own
    # ACTUAL fretboard position for its own onset pitch (via
    # _fd_positions_for_pitch(), REUSED unmodified, first match --
    # the same convention Rule #1 itself already uses at a chord
    # onset). This is a genuinely different question from the
    # existing following_working_fret_anchor above (that dict
    # only ever holds a single IMMEDIATELY-adjacent chord's own
    # working fret; a note several notes away from the next chord
    # onset needs the wider lookup, and BO-40's own trigger
    # condition specifically needs the chord's own onset note's
    # REAL string, not just its working fret, to measure a
    # genuine string-distance -- confirmed via direct
    # investigation that comparing against working_fret alone
    # would not correctly distinguish the real measure-22 example
    # from every other G4 occurrence in the song).
    next_chord_onset_position_by_event_id = {}

    onset_pitch_by_position = {}

    for harmony in staff_harmonies:

        onset_notes = _melody_notes_at_harmony_onset(
            harmony, score_file.score.notes
        )
    ...
    next_chord_onset_key_by_index = [None] * len(
        flat_note_events
    )

    next_key = None

    for index in range(len(flat_note_events) - 1, -1, -1):
        ...
        if position_key in chord_shape_by_position:
            ...
            next_key = position_key

        next_chord_onset_key_by_index[index] = next_key
    ...
    for index, (measure_number, event) in enumerate(
        ...
        if position_key in chord_shape_by_position:
            ...
            continue  # at a chord onset -- not a candidate note

        onset_key = next_chord_onset_key_by_index[index]

        if onset_key is None:

            continue

        onset_pitch = onset_pitch_by_position.get(onset_key)

        if onset_pitch is None:

            continue

        onset_shape_values = chord_shape_by_position.get(
            onset_key
        )

        if onset_shape_values is None:

            continue

        # BO-40 bug found and fixed during validation: a raw
        # _fd_positions_for_pitch()[0] call here (first match by
        # string_index order) does NOT reproduce the real
        # position the TAB writer will actually use, when the
        # onset pitch sounds on more than one string within the
        # chord's own shape -- BO-35 already resolves that
        # specific situation by picking the CLOSEST occurrence to
        # the pitch's own preferred fret, not the first by index.
        # Confirmed via a real, concrete case: Ddim's own shape
        # (5780) sounds D4 at BOTH string_index 1/fret 7 and
        # string_index 3/fret 0 -- the raw first-match approach
        # here initially reported string_index 1, string_
        # distance 2 away from a real, unrelated G4's own normal
        # choice, incorrectly triggering BO-40 there, when the
        # REAL onset position (string_index 3, matching BO-35's
        # own logic, since D4's own lowest available fret is 0)
        # is only string_distance 0 away -- no genuine transition
        # need at all. Calling _choose_melody_position() directly
        # here, exactly as the real TAB-writing loop below does
        # for this same onset note, reuses that same, correct
        # logic instead of a second, diverging approximation of
        # it.
        onset_chosen = _choose_melody_position(
            onset_pitch, open_notes,
            fd_shape_values=onset_shape_values
        )

        if onset_chosen is not None:

            next_chord_onset_position_by_event_id[id(event)] = (
                onset_chosen["string"]
            )
    ...
    for measure_index, measure_events in enumerate(measures):
        ...
        for event in measure_events:
            ...
            # BO-40: a narrow, separate transition mechanism --
            # NOT a normal melody-position candidate, and NOT
            # folded into _choose_melody_position()'s own sort
            # key at all (per explicit instruction: the 5th
            # string must never enter find_positions() or
            # participate in the existing string-distance/
            # pattern-continuity calculations). Applied only
            # AFTER the normal four-string choice is already
            # made, and only when ALL THREE conditions hold:
            #   1. this note's own exact pitch equals tuning.
            #      notes[0] (the open 5th string's own pitch) --
            #      the ONLY way this can ever apply, so a
            #      different pitch (e.g. the very next F4 in the
            #      real measure-22 example) can never be silently
            #      substituted for a convenient open string.
            #   2. this note is not itself a chord onset (Rule #1
            #      / FD compatibility remains completely
            #      untouched -- this check never even runs for an
            #      onset note).
            #   3. the normal four-string choice just computed
            #      above is genuinely far (string_distance >= 2)
            #      from the next chord onset's own REAL position
            #      -- confirmed via direct investigation across
            #      every one of the 19 real non-onset G4
            #      occurrences in the whole song that this
            #      threshold uniquely identifies the real,
            #      confirmed measure-22 case and correctly
            #      excludes every other one, including a second
            #      candidate (measure 31) that turned out to
            #      already be well-positioned once checked
            #      precisely (string_distance 0, not a genuine
            #      transition need at all).
            # When BO-40 applies, string=4 (the literal 5th-
            # string MuseScore value, confirmed against a real,
            # genuine MuseScore-created reference file -- see
            # this function's own module-level docstring) and
            # fret=0 (open -- BO-40's own scope is specifically
            # the open 5th string, matching condition 1's exact-
            # pitch requirement, which can only ever be satisfied
            # by the open string itself in this tuning). previous_
            # melody_position/second_previous_melody_position are
            # DELIBERATELY left unchanged below (not updated to
            # this note's own 5th-string result) -- the next
            # note's own string-continuity/pattern-continuity
            # calculations see the note BEFORE this one, exactly
            # as if this transition note were not part of that
            # calculation at all, per explicit instruction.
            next_onset_string_index = (
                next_chord_onset_position_by_event_id.get(
                    id(event)
                )
            )
            ...
            if (
                midi == tuning.notes[0]
                and not is_chord_onset
                and next_onset_string_index is not None
                and abs(
                    chosen["string"] - next_onset_string_index
                ) >= 2
            ):
            ...
            # BO-23-FOLLOWUP's own investigation notes). BO-40:
            # the 3-idx formula is only valid for the 4 melody
            # strings (0-3) -- it does not apply to the 5th
            # string at all (3-4=-1, invalid). Confirmed against
            # a real, genuine MuseScore-created reference file
            # (The Christmas Song (with TAB and composer).mscz,
            # its own original TAB before BO-17's own "strip the
            # banjo part" process) that MuseScore writes the 5th
            # string as the literal value 4 directly -- not
            # derived from any reversal formula at all.
            if chosen["string"] == 4:
    ...
    chord_shapes_applied, chord_shapes_skipped, exceptions = (
        _apply_chord_shapes(
            ...
            melody_notes=score_file.score.notes,
            precomputed_selection_by_position=(
                second_pass_selection_by_position
            )
        )
    )
