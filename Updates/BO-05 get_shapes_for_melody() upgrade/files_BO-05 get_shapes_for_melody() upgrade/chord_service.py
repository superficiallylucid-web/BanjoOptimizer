...
from models import MelodyRealizationMatch, MelodyRealizationDiagnostic
...
class ChordService:
    ...
    def get_shapes_for_melody(
        ...
        Ordering rule: every shape that sounds melody_note
        anywhere comes first, followed by every shape that
        doesn't -- but WITHIN each of those two groups, the
        relative order from get_shapes() is preserved exactly.
        This is a stable partition, not a new score: it
        deliberately does not re-rank verified vs. generated
        shapes against each other beyond what matching/non-
        matching decides. A matching generated shape can end up
        ahead of a non-matching verified shape (matching is
        evaluated first), but two shapes that both match, or
        both don't, keep whatever order get_shapes() already
        gave them.
        """
