...
def test_full_song_every_measure_duration_correct():
    ...
    try:
        ...
        for note in staff.findall(".//{*}Note"):
            ...
            # BO-40: the 5th string (MuseScore string=4) is not
            # part of the 3-idx reversal formula at all -- its
            # own open pitch is A_MODAL_SAWMILL.notes[0], not a
            # fretboard_string_index lookup into open_notes.
            if ms_string == 4:
                ...
                computed = (
                    open_notes[fretboard_string_index] + fret
                )
